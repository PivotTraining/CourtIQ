...
- **Paid-tier bypass:** Pro and Team IQ were granted entirely by `localStorage`, and the purchase button simulated a successful subscription. Client-side grants are removed and purchasing is disabled until StoreKit/Play Billing receipts are verified server-side.
...
2. Replace the placeholder paid-tier screen with App Store / Play Billing plus server-verified entitlements, or remove all paid-tier marketing from the submitted binary.
3. Inspect App Store Connect build 64's processing errors while signed in. Reconcile the latest Apple build number before archiving.
4. Select/install full Xcode, download the Apple team profiles, archive build 65 with automatic signing, and run it on a supported physical iPhone. The current machine exposes Command Line Tools only, so an Xcode compile/archive could not be completed here.
5. Verify Supabase OAuth redirect allowlists include the existing native scheme and production web callback. Test email/password, password reset, Google, and Apple sign-in on device.
6. Deploy and test the `delete-account` Edge Function, then validate the public privacy policy/support contact against actual retention and child-consent practices.
